...
class WIWH_BC:
    ...
    def _count_yc_days(self, daily, sowing_text: Any, mature_text: Any) -> float:
        if daily is None or len(daily) == 0:
            return np.nan
        cnt = 0
        ok_all = (daily['tavg'] >= 12) & (daily['tavg'] <= 22) & (daily['rhum'] < 70)
    ...
    def _compute_station_values(self, station_coords: Dict[str, Any], cfg: Dict[str, Any], algo_cfg: Dict[str, Any]) -> Dict[str, float]:
        ...
        if usable_ids:
            ...
            with Pool(16) as pool:
                results = pool.map(_yc_worker, args)
    ...
    def _interpolate(self, station_values: Dict[str, float], station_coords: Dict[str, Any], params: Dict[str, Any]):
        ...
        out_tif = intermediate_dir / "intermediate_YC_days.tif"
    ...
    def calculate_YC(self, params: Dict[str, Any]) -> Dict[str, Any]:
        self._algorithms = params['algorithms']
        station_coords = params.get('station_coords', {})
        algo_cfg = params.get('algorithmConfig', {})
        cfg = params.get('config', {})
        print("开始计算冬小麦蚜虫发病适宜日数并插值")
        station_values = self._compute_station_values(station_coords, cfg, algo_cfg)
        print("站点多年平均适宜日数统计完成")
        try:
            intermediate_dir = Path(cfg["resultPath"]) / "intermediate"
            intermediate_dir.mkdir(parents=True, exist_ok=True)
            out_csv = intermediate_dir / "intermediate_WIWH_YC.csv"
            df_station = pd.DataFrame(
                [{"Station_Id_C": k, "YC_days": v} for k, v in station_values.items()]
            )
            df_station.to_csv(out_csv, index=False, encoding="utf-8-sig")
            print(f"站点YC适宜日数已保存到: {out_csv}")
        except Exception as e:
            print(f"保存站点YC适宜日数CSV失败: {e}")
...
def _yc_worker(args):
    ...
    helper = WIWH_BC()
    yc = helper._count_yc_days(daily, sowing_text, mature_text)
    return sid_str, yc
