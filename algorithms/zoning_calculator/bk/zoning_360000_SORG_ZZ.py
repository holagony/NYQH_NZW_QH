...
class SORG_ZZ:
    ...
    def __call__(self, params):
        return self.calculate(params)  # 转发调用
    ...
    def _calculate_element(self, params):
        ...
        if classification_method == 'custom_thresholds' and len(result['data']) > 1:
            # 多指标保守分类
            data = self._conservative_classification(result['data'], classification)
            result['data'] = data
        else:
            # 单指标分类（保持原有逻辑）
            classifier = self._get_algorithm(f"classification.{classification_method}")
            first_indicator = list(result['data'].keys())[0]
            data = classifier.execute(result['data'][first_indicator], classification)
            result['data'] = data
    ...
    def _conservative_classification(self, data_dict, classification_config):
        """
        保守策略分类：对多个指标数据，选择最不适宜的等级

        Args:
            data_dict: 字典，包含多个指标数据，如 {'AT10': array, 'Tmin': array}
            classification_config: 分类配置参数
        ...
        # 获取所有指标的阈值配置
        threshold_configs = {}
        for key in classification_config.keys():
            if key.startswith('thresholds_'):
                indicator_name = key.replace('thresholds_', '')
                threshold_configs[indicator_name] = classification_config[key]
        ...
        # 对每个有阈值配置的指标进行分类
        for indicator_name, thresholds in threshold_configs.items():
            if indicator_name in data_dict:
                indicator_data = data_dict[indicator_name]['data']

                # 创建分类结果数组
                classified_data = np.full_like(indicator_data, 1, dtype=np.int32)  # 默认最适宜等级1

                # 应用自定义阈值分类
                for i in range(indicator_data.shape[0]):
                    for j in range(indicator_data.shape[1]):
                        value = indicator_data[i, j]
                        ...
                        for threshold in thresholds:
                            ...
                            if min_val <= value < max_val:
                                classified_data[i, j] = level
                                break

                classified_results[indicator_name] = classified_data

        # 如果没有找到匹配的指标，返回第一个指标的数据
        if not classified_results:
            first_key = list(data_dict.keys())[0]
            return data_dict[first_key]

        # 获取数据形状
        first_key = list(classified_results.keys())[0]
        output_shape = classified_results[first_key].shape

        # 创建最终结果数组，初始化为最适宜等级
        conservative_result = np.full(output_shape, 1, dtype=np.int32)

        # 对每个位置，选择所有指标中最差的等级（等级数字越大越不适宜）
        for i in range(output_shape[0]):
            for j in range(output_shape[1]):
                worst_level = 1  # 从最适宜等级开始

                for indicator_name, classified_data in classified_results.items():
                    level = classified_data[i, j]
                    # 选择等级数字最大的（等级数字越大越不适宜）
                    if level > worst_level:
                        worst_level = level

                conservative_result[i, j] = worst_level
