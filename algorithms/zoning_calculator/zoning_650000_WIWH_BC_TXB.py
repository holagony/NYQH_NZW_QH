...
def count_comfortable_days_TXB(daily_data,start_date,end_date):
    """
    计算日平均温度为9～20℃且降水量大于等于5mm的平均日数
    """
    mask_date=((daily_data.index.month==int(start_date[:2]))&(daily_data.index.day>=int(start_date[4:5])))|((daily_data.index.month==int(end_date[:2]))&(daily_data.index.day<=int(end_date[4:5])))    
    tavg_level=[9,20]
    pre_level=[0.5,]
    # 从DataFrame中提取符合要求的数据
    comfortable_df = daily_data[(daily_data["tavg"]>=tavg_level[0])& (daily_data["tavg"]<=tavg_level[1])&
                    (daily_data["precip"]>=pre_level[0])&mask_date]
...
class WIWH_BC:
    ...
    def calculate_TXB(self, params):
        ...
        for sid in station_ids:
            ...
            station_value=count_comfortable_days_TXB(daily,start_date,end_date)
        ...
        tif_path = os.path.join(intermediate_dir, "条锈病未分级栅格数据.tif")
