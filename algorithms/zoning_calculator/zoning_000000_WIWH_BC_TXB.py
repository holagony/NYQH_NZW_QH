...
class WIWH_BC:
    ...
    def _count_txb_days(self, daily, sowing_text: Any, mature_text: Any) -> float:
        if daily is None or len(daily) == 0:
            return np.nan
        cnt = 0
        ok_all = (daily['tavg'] >= 9) & (daily['tavg'] <= 20) & (daily['precip'] > 0.5)
    ...
    def _compute_station_values(self, station_coords: Dict[str, Any], cfg: Dict[str, Any], algo_cfg: Dict[str, Any]) -> Dict[str, float]:
        ...
        if usable_ids:
            ...
            with Pool(16) as pool:
                results = pool.map(_txb_worker, args)
...
def _txb_worker(args):
    ...
    helper = WIWH_BC()
    yc = helper._count_txb_days(daily, sowing_text, mature_text)
    return sid_str, yc
