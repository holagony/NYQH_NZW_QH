...
class WIWH_BC:
    ...
    def _count_cmb_days(self, daily, sowing_text: Any, mature_text: Any) -> float:
        if daily is None or len(daily) == 0:
            return np.nan
        cnt = 0
        ok_all = (daily['tavg'] >= 15) & (daily['precip'] > 0.1)
    ...
    def _compute_station_values(self, station_coords: Dict[str, Any], cfg: Dict[str, Any], algo_cfg: Dict[str, Any]) -> Dict[str, float]:
        ...
        if usable_ids:
            ...
            with Pool(16) as pool:
                results = pool.map(_cmb_worker, args)
    ...
    def _interpolate(self, station_values: Dict[str, float], station_coords: Dict[str, Any], params: Dict[str, Any]):
        ...
        out_tif = intermediate_dir / "intermediate_CMB_days.tif"
    ...
    def calculate_CMB(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            intermediate_dir = Path(cfg["resultPath"]) / "intermediate"
            intermediate_dir.mkdir(parents=True, exist_ok=True)
            out_csv = intermediate_dir / "intermediate_WIWH_CMB.csv"
            df_station = pd.DataFrame(
                [{"Station_Id_C": k, "CMB_days": v} for k, v in station_values.items()]
            )
            df_station.to_csv(out_csv, index=False, encoding="utf-8-sig")
            print(f"站点CMB适宜日数已保存到: {out_csv}")
    ...
    def calculate(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if element == 'CMB':
            return self.calculate_CMB(params)
...
def _cmb_worker(args):
    ...
    helper = WIWH_BC()
    yc = helper._count_cmb_days(daily, sowing_text, mature_text)
    return sid_str, yc
