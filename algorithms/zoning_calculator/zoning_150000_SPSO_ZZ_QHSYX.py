...
class SPSO_ZZ:
    ...
    def calculate(self, params): # 修改
        ...
        if type == 'QHSYX':
            ...
            raise ValueError(f"不支持的区划类型: {type}")
    ...
    def _calculate_protein(self, params):
        ...
        protein_indicators = self._calculate_protein_indicators_station(station_indicators)

        # 第二步：在站点级别计算食心虫综合风险指数F
        print("第二步：在站点级别计算蛋白质气候区划指数F")
        protein_station = self._calculate_protein_station(protein_indicators, algorithmConfig)

        # 第三步：对综合风险指数F进行插值
        print("第三步：对蛋白质气候区划指数F进行插值")
        interpolated_risk = self._interpolate_protein(protein_station, station_coords, config,
                                                             algorithmConfig)
    ...
    def _calculate_protein_indicators_station(self, station_indicators):
        ...
        for station_id, indicators in station_indicators.items():
            ...
            protein_indicators[station_id] = station_data

        # 统计各指标的有效站点数
        self._print_station_indicator_stats(protein_indicators)

        return protein_indicators

    def _calculate_protein_station(self, protein_indicators, crop_config):
        """在站点级别计算蛋白质气候区划指数F"""
        # 获取配置中的公式
        ...
        for station_id, indicators in protein_indicators.items():
            ...
            if not np.isnan(X1) and not np.isnan(X2) and not np.isnan(X3) and not np.isnan(X4):
                ...
                try:
                    ...
                    protein[station_id] = F
                    valid_count += 1
                except Exception as e:
                    print(f"站点 {station_id} 蛋白质气候区划指数计算失败: {str(e)}")
                    protein[station_id] = np.nan
            else:
                protein[station_id] = np.nan

        print(f"站点蛋白质气候区划指数计算完成，有效站点数: {valid_count}/{len(protein_indicators)}")

        # 统计蛋白质气候区划指数的范围
        if valid_count > 0:
            values = [v for v in protein.values() if not np.isnan(v)]
            min_val = min(values)
            max_val = max(values)
    ...
    def _calculate_before_interpolate(self, station_indicators, station_coords,
        ...
        for station_id, station_values in station_indicators.items():
            try:
                ...
                # 在站点级别计算综合指标
                composite_value = self.calculate_composite_indicator(station_indicators_dict, crop_config)
    ...
    def calculate_composite_indicator(self, interpolated_indicators: Dict[str, Any],
        ...
        if var_type == "standardize":
            ...
            if "ref" in value_config:
                ...
                if ref_name in interpolated_indicators:
                    ...
                    return self.dataNormal_array(grid_data)
                else:
    ...
    def dataNormal_tif(self,infile=None, outfile=None, nodata=-999):
        """
        栅格数据归一化
        ...
        try:
            ...
            # outdata = data
            outdata[(data!=srs_nodata)&(data!=0)] = (data[(data!=srs_nodata)&(data!=0)]-min_value)/(max_value-min_value)
            outdata[(data == 0)] =0
    ...
    def dataNormal_array(self,data,nodata=np.nan):
        """
        要素归一化处理
        ...
        max_value = np.nanmax(data[(data!=nodata)])
        min_value = np.nanmin(data[(data!=nodata)])
        # outdata = data
        outdata[(data!=nodata)] = (data[(data!=nodata)]-min_value)/(max_value-min_value)
        outdata[np.isnan(outdata)] = 0
        # outdata[outdata == np.inf] = 0
        return outdata
