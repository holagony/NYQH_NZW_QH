...
MODEL_PATH = PROJECT_ROOT / "test" / "lgb_zz.pkl"
...
class SPSO_ZZ:
        ...
        for y in years:
            ...
            for m in range(5, 10):
                ...
                if month_data.empty:
                    ...
                    t_val = month_data["tavg"].mean() if "tavg" in month_data.columns else np.nan
                    p_val = month_data["precip"].sum() if "precip" in month_data.columns else np.nan
                    r_val = month_data["sunshine"].sum() if "sunshine" in month_data.columns else np.nan
                row[f"T{m}"] = t_val
                row[f"P{m}"] = p_val
                row[f"R{m}"] = r_val
            row["lat"] = float(sub["lat"].iloc[0]) if "lat" in sub.columns else np.nan
            row["lon"] = float(sub["lon"].iloc[0]) if "lon" in sub.columns else np.nan
            rows.append(row)
        if not rows:
            return pd.DataFrame()
        cols = [
    ...
    def calculate_drought(self, params):
        ...
        data_out = classifier.execute(norm_data.astype(float), class_conf)
        final_tif = str(out_dir / "适宜度指数_分级.tif")
