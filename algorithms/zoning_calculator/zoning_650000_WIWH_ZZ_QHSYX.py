...
class WIWH_ZZ:
    ...
    def _interpolate_before_calculate(self, station_indicators, station_coords,
        ...
        condition_1=(interpolated_indicators["acc_tmp"]["data"]>=2300)&(interpolated_indicators["tmin_extr"]["data"]>=(-30))&(interpolated_indicators["sd_max"]["data"]>=10)
        #不适宜
        ...
        suitability_indicators=np.where(condition_2,2.5,suitability_indicators)     #次适宜
        suitability_indicators=np.where(condition_3,1.5,suitability_indicators)     #适宜
        suitability_indicators=np.where(condition_4,0.5,suitability_indicators)     #最适宜
        suitability_indicators=np.where(~condition_1,3.5,suitability_indicators)     #不适宜
        result={
                'data': suitability_indicators,
                'meta':interpolated_indicators["acc_tmp"]["meta"]
            }
        return result
