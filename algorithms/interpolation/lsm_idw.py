        ...
        try:
            ...
            if min_value is not None:
                try:
                    ...
                    final_grid[final_grid < min_val] = min_val
                    print(f"应用最小值限制: {min_val}")
                except (ValueError, TypeError):
                    print(f"警告: 最小值参数无法转换为数值: {min_value}")
            
            if max_value is not None:
                try:
                    max_val = float(max_value)
                    final_grid[final_grid > max_val] = max_val
                    print(f"应用最大值限制: {max_val}")
                except (ValueError, TypeError):
                    print(f"警告: 最大值参数无法转换为数值: {max_value}")
