    ...
    def _print_indicator_stats(self, results_df: pd.DataFrame, indicator_configs: Dict[str, Any]) -> None:
        ...
        for indicator in indicator_names:
            if indicator in results_df.columns:
                ...
                if len(valid_values) > 0:
                    # 判断指标类型
                    if self._is_text_indicator(indicator):
                        # 文本型指标：输出城市列表
                        unique_cities = valid_values.unique()
                        city_list = ', '.join([str(city) for city in unique_cities[:10]])  # 只显示前10个
                        if len(unique_cities) > 10:
                            city_list += f" ... 等{len(unique_cities)}个城市"
                        print(f"  {indicator}: 有效值{len(valid_values)}个, 城市: {city_list}")
                    else:
                        # 数值型指标：计算统计信息
                        min_val = valid_values.min()
                        max_val = valid_values.max()
        ...
        try:
            ...
            for indicator_name, indicator_config in indicator_configs.items():
                try:
                    ...
                    if indicator_name in ['City']: # 注意是指标的名字
                        station_results[indicator_name] = station_results[indicator_config['variable']]                      # print(data[indicator_config['variable']].dropna().unique())
                    else:
